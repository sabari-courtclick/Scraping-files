...
import { saveCaseData } from "./db.js";
...
async function solveCaptcha(page) {
    ...
    let attempts = 0;
    const maxAttempts = 30; // 30 attempts with 5 seconds interval = 150 seconds

    while (!captchaText && attempts < maxAttempts) {
    ...
    if (!captchaText) {
        throw new Error("Failed to solve CAPTCHA within the allowed time");
    }
