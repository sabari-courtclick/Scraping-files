      ...
      try {
        ...
        if (responseText.includes('app_token')) {
          ...
          if (captchaResponse.status === 200 && captchaResponse.headers['content-type'].startsWith('image/')) {
            ...
            const originalImagePath = `debug_captcha_original_${timestamp}.png`;
            const processedImagePath = `debug_captcha_processed_${timestamp}.png`;
            
            fs.writeFileSync(originalImagePath, captchaResponse.data);
            console.log(`[DEBUG] Original CAPTCHA saved to: ${originalImagePath}`);
            console.log(`[DEBUG] Content-Type: ${captchaResponse.headers['content-type']}`);
            console.log(`[DEBUG] Image size: ${captchaResponse.data.length} bytes`);
            
            // Basic image processing
            const captchaBuffer = Buffer.from(captchaResponse.data);
            console.log('[DEBUG] Converting image to buffer...');
            ...
            try {
                ...
                console.log(`[DEBUG] Processed image size: ${processedImage.length} bytes`);
                
                // Save processed image
                await sharp(processedImage).toFile(processedImagePath);
                console.log(`[DEBUG] Processed CAPTCHA saved to: ${processedImagePath}`);

                // OCR with debug info
                console.log('[DEBUG] Starting OCR...');
                const worker = await createWorker();
                await worker.loadLanguage('eng');
                await worker.initialize('eng');
                
                console.log('[DEBUG] Setting OCR parameters...');
                ...
                console.log('[DEBUG] Running OCR on processed image...');
                const { data: { text, confidence } } = await worker.recognize(processedImage);
                console.log(`[DEBUG] Raw OCR output: "${text}"`);
                console.log(`[DEBUG] OCR confidence: ${confidence}%`);
                
                await worker.terminate();
                console.log('[DEBUG] OCR completed');
                ...
                console.log(`[DEBUG] Cleaned CAPTCHA text: "${captchaText}"`);
                console.log(`[DEBUG] Text length: ${captchaText.length}`);
                console.log(`[DEBUG] Contains only allowed chars: ${/^[a-zA-Z0-9]+$/.test(captchaText)}`);

                if (captchaText && captchaText.length >= 4) {
                    console.log(`[DEBUG] CAPTCHA validation passed`);
                    return { success: true, captchaText };
                } else {
                    console.warn(`[DEBUG] CAPTCHA validation failed - Text too short or empty`);
                }
            } catch (error) {
                console.error('[DEBUG] Error during image processing/OCR:', error.message);
                throw error;
            }
          } else {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if (result.errormsg) {
            ...
            // If it's an invalid CAPTCHA error and we have a new app token
            if (result.errormsg.includes('Invalid Captcha') && result.app_token) {
              console.log('Received new app token after invalid CAPTCHA:', result.app_token);
              this.appToken = result.app_token;
              
              // Don't increment attempt counter since we got a valid response with new token
              continue;
            }
